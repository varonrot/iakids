...
def page_timeline(c):
    ...
    p.h2('שלושה שיפורים שהלוג חשף, בלי קשר לבנייה מוקדמת (מומשו 15.9 אחה"צ)')
    p.p('1. Director של חלק 1 רץ במקביל למורה של חלק 2 (asyncio.create_task): חוסך ~16 שניות מכל שיעור חדש (77 → ~60). מומש.')
    p.p('2. אודיו פרוגרסיבי: המסלול מחזיר partial וה־workspace מנגן את חלק 1 ברגע שהוא מוכן (T+73 במקום T+151). כבר פעיל בקוד.')
    p.p('3. TTS ב־3 קריאות במקביל לכל חלק (TTS_PARALLEL=3), תוצאות נצרכות לפי סדר: 147 → ~55 ש׳. מומש; הרצה של שיעור 30 מודדת.')
