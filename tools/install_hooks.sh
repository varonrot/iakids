...
if git diff --cached --name-only | grep -qE '^(backend-ai-tutor-he/(prompts/.*\.txt|[^/]+\.py)|backend/main\.py|tools/security_gate_[a-z]+\.py|supabase/migrations/[^/]+\.sql|support/index\.html|support-dashboard/index\.html|games/game-sdk\.js|he/workspace/[^/]+\.js|he/parent-panel/index\.html|he/workspace/index\.html|tools/prompt_gate\.py|performance/(routes|static_checks)\.py|performance/results/latest-fake\.json)$'; then
