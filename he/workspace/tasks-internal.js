/* IAKIDS internal tasks view 0.7.66 */
(function(){
  if(window.__IAKIDS_INTERNAL_TASKS_0766) return;
  window.__IAKIDS_INTERNAL_TASKS_0766 = true;
  ...
  function ensureStyles(){
    ...
    style.textContent=`
      ...
      .iit-toolbar{
        ...
        padding:0 14px;
        background:linear-gradient(180deg,#071425,#04101e);
        border-bottom:1px solid rgba(92,154,232,.18);
        color:#eef6ff;
      }
      .iit-toolbar strong{font:900 17px "Heebo",Arial,sans-serif;}
      .iit-toolbar strong i{color:#6ddcff;margin-left:7px;}
      .iit-close{
        height:36px;padding:0 13px;border:1px solid rgba(89,154,235,.28);border-radius:11px;
        background:#0a1b31;color:#cfe3fb;font:800 13px "Heebo",Arial,sans-serif;cursor:pointer;
      }
  ...
  function applyEmbeddedDarkTheme(doc){
    ...
    if(mainInner){
      mainInner.style.paddingTop='16px';
      mainInner.style.paddingBottom='24px';
    }

    doc.documentElement.style.background='#020b18';
    doc.body.style.background='#020b18';

    if(doc.getElementById('iakidsEmbeddedTasksDarkTheme')) return;
    const style=doc.createElement('style');
    style.id='iakidsEmbeddedTasksDarkTheme';
    style.textContent=`
      :root{
        --purple:#6b4dff!important;
        --purple2:#8a68ff!important;
        --ink:#edf5ff!important;
        --muted:#8fa6c6!important;
        --line:rgba(83,140,214,.22)!important;
        --bg:#020b18!important;
        --card:#07182b!important;
        --shadow:0 16px 36px rgba(0,0,0,.28)!important;
        --soft:0 8px 22px rgba(0,0,0,.20)!important;
      }
      ...
      .hero-copy h1,.calendar-title,.tasks-head h2,.kid-meta strong{
        color:#eef6ff!important;
      }
      .hero-copy p,.kid-meta small,.stat small,.task-sub,.empty,.weekday{
        color:#8fa6c6!important;
      }
      .card,.calendar-card,.kid-card,.stats,.filters,.tasks-card{
        background:linear-gradient(180deg,rgba(8,25,46,.98),rgba(4,16,31,.98))!important;
        border:1px solid rgba(86,151,231,.22)!important;
        box-shadow:0 16px 36px rgba(0,0,0,.24)!important;
      }
      .kid-card{
        background:linear-gradient(145deg,#0a1e35,#08162a)!important;
      }
      .stat{
        background:#0a1c32!important;
        color:#f2f7ff!important;
      }
      .filter-btn,.icon-btn,.task-action{
        background:#0a1c32!important;
        border-color:rgba(85,145,220,.24)!important;
        color:#adc4df!important;
      }
      .filter-btn:hover,.icon-btn:hover,.task-action:hover{
        background:#102845!important;
        color:#fff!important;
      }
      .filter-btn.active{
        background:linear-gradient(135deg,#243b84,#5030bc)!important;
        border-color:#6e7dff!important;
        color:#fff!important;
      }
      .day{
        background:#07182b!important;
        border-color:rgba(82,137,205,.20)!important;
        color:#dfeeff!important;
      }
      .day:hover{
        background:#0b2039!important;
        border-color:#438fe7!important;
      }
      .day.today{
        background:#10254b!important;
        border-color:#5ed9ff!important;
      }
      .day.selected{
        box-shadow:inset 0 0 0 2px rgba(102,128,255,.55)!important;
      }
      .task{
      ...
      .task-title{color:#edf5ff!important;}
      .add-btn,.save-btn{
        background:linear-gradient(135deg,#2377e8,#5a38d7)!important;
        box-shadow:0 10px 24px rgba(22,98,214,.28)!important;
      }
      .modal-backdrop{background:rgba(0,5,14,.72)!important;}
      .modal{
        background:#07182b!important;
        border:1px solid rgba(85,145,220,.26)!important;
        color:#edf5ff!important;
      }
      .modal-head h2,.field label{color:#edf5ff!important;}
      .field input,.field select,.field textarea{
        background:#0a1c32!important;
        border-color:rgba(85,145,220,.28)!important;
        color:#edf5ff!important;
      }
      .close-btn,.cancel-btn{
        background:#0a1c32!important;
        border-color:rgba(85,145,220,.24)!important;
        color:#c7dbf2!important;
      }
      .calendar-card,.tasks-card,.filters,.stats,.kid-card{backdrop-filter:blur(8px);}
    `;
  ...
  function getView(){
    ...
    frame?.addEventListener('load',()=>{
      try{
        applyEmbeddedDarkTheme(frame.contentDocument);
      }catch(error){
