...
def homework_plan_block(page_summary: str, question_plan: dict) -> str:
    ...
    return (
        ...
        f"Steps, in order — continue from the first step the child has not completed yet:\n{steps}\n"
...
async def plan_homework_page(analysis: dict, grade) -> dict:
    """Stage 2: one strong-model call that plans every question on the page. Retries once if a
    plan puts the answer in what the teacher says; still leaking → those steps are dropped."""
    ...
    user_content = json.dumps({"grade": grade, "page": page}, ensure_ascii=False)
    plan = None
    for attempt in (1, 2):
        completion = await aclient.beta.chat.completions.parse(
            model=UNIVERSAL_LESSON_MODEL,
...
async def _run_homework_planner(upload_id: str, analysis: dict, grade):
    started = time.time()
    try:
        plan = await plan_homework_page(analysis, grade)
        merged = {**analysis, "teaching_plan": plan, "plan_status": "ready"}
        print("HOMEWORK PLAN READY:", {"upload_id": upload_id, "questions": len(plan["questions"]),
                                       "leaks_removed": plan["leaks_removed"],
                                       "seconds": round(time.time() - started, 1)})
    except Exception as e:
        merged = {**analysis, "plan_status": "failed"}
...
async def homework_plan_block_for(user_id: str, upload_id: str | None, current_question: str) -> str:
    ...
    while True:
        ...
        if status != "pending" or time.time() >= deadline:
            break
