...
- `nikud.js` + `IAKidsNikud` (in `game-sdk.js`) — 3,739 Hebrew words mapped to the same word with vowel points (identical letters). A game keeps its plain words for all logic and renders the vocalised one. Generated by `games/tools/nakdan.py`, mirrored into Supabase by `backend/seed_nikud.py`.
