...
- `IAKidsSpeech` (in `game-sdk.js`) — read-aloud for every game: cache first, then the browser's own voice, and only as a last resort the model voice, whose audio is then cached so it is generated once. A floating 🔊 that reads the current question is mounted automatically.
- `nikud.js` + `IAKidsNikud` (in `game-sdk.js`) — 3,928 Hebrew words mapped to the same word with vowel points (identical letters). A game keeps its plain words for all logic and renders the vocalised one; titles, how-to text and buttons are vocalised automatically because `IAKidsLang.t()` passes Hebrew through it. A pill in the start screen turns nikud on or off, defaulting from the child's age. Generated by `games/tools/nakdan.py`, mirrored into Supabase by `backend/seed_nikud.py`.
