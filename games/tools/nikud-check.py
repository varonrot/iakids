...
words this checker then demands coverage of — the two can never drift apart:
    ...
    python3 games/tools/nikud-build.py /tmp/n.json      # master + one small file per page
...
# Only the combining vowel and cantillation marks count as "vocalised". The
# maqaf (־) and the other Hebrew punctuation in that block are just punctuation:
# "מתאימה ל־" is not nikud, and flagging it made the R7 warning noise.
COMBINING = re.compile('[\u0591-\u05bd\u05bf\u05c1\u05c2\u05c4\u05c5\u05c7]')
...
# Games that teach the marks themselves: hiding them would remove the lesson.
NIKUD_IS_THE_SUBJECT = {'nikud', 'syllables'}
...
def load_dictionary():
    src = open(f'{ROOT}/tools/nikud.full.js', encoding='utf-8').read()   # the master; pages load their own split
...
def main():
    ...
    # R4b — the split: every page's own nikud.js must hold every word that page renders.
    # The master can be complete while a page's file is stale (built before the game
    # changed), and then the child sees bare words beside pointed ones.
    stale = []
    for slug, c in per_game.items():
        if slug == 'game-sdk': continue
        path = f'{ROOT}/nikud.js' if slug == 'games' else f'{ROOT}/{slug}/nikud.js'
        if not os.path.isfile(path): continue
        own = set(re.findall(r"'([א-ת]+)':", open(path, encoding='utf-8').read()))
        need = {w for w in c if w in dictionary} | {w for w in per_game.get('game-sdk', {}) if w in dictionary}
        missing = need - own
        if missing:
            stale.append(f'{slug} ({len(missing)}: {" ".join(sorted(missing)[:4])})')
    if stale:
        fail('R4b split', f'{len(stale)} page files are behind the master — run nikud-build.py: ' + ', '.join(stale[:6]))
    ...
    if 'GENERATED' not in src.split('const IAKIDS_NIKUD')[0]:
        fail('R6 generated', 'nikud.js lost its "generated" header — is it being edited by hand?')

    # R7 — vocalised text inside a game must still answer to the pill.
    #
    # A hand-written vocalised table is not itself the problem; a table that renders
    # whatever the child chose is. So the test is not "does this game hold marks" but
    # "does it ask IAKidsNikud.enabled before showing them".
    baked = []
    for f in sorted(glob.glob(f'{ROOT}/*/index.html')):
        s = open(f, encoding='utf-8').read()
        blocks = re.findall(r'<script>(.*?)</script>', s, re.S)
        has_marks = any(re.search('[א-ת]' + COMBINING.pattern, b) for b in blocks)
        slug = os.path.basename(os.path.dirname(f))
        # Exempt: a game whose subject *is* nikud has to show marks whatever the
        # pill says, and IAKidsNikud.local() is the sanctioned way to pin a reading.
        asks_pill = ('IAKidsNikud.enabled' in s or 'IAKidsNikud.local(' in s
                     or slug in NIKUD_IS_THE_SUBJECT)
        if has_marks and not asks_pill:
            sample = None
            for b in blocks:
                sample = re.search("['\"]([^'\"\\n]{0,20}[א-ת]" + COMBINING.pattern + "[^'\"\\n]{0,20})['\"]", b)
                if sample: break
            baked.append(os.path.basename(os.path.dirname(f)) +
                         (f' ({sample.group(1)})' if sample else ''))
    if baked:
        warn('R7 no baking', f'{len(baked)} games show vocalised text without asking the pill '
                             f'(IAKidsNikud.enabled): ' + ', '.join(baked[:8]))

    # R8 — a game that renders a word must load the dictionary.
    for f in sorted(glob.glob(f'{ROOT}/*/index.html')):
        s = open(f, encoding='utf-8').read()
        if 'IAKidsNikud' in s and not re.search(r'src="nikud\.js', s):
            fail('R8 script tag', f'{os.path.basename(os.path.dirname(f))} calls IAKidsNikud '
                                  f'but never loads its own nikud.js (run nikud-build.py)')
