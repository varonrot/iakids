 ...
 *
 * The dictionary (games/nikud.js, generated by games/tools/nakdan.py) maps a
 * plain word to the same word with nikud: same letters, marks added. A game
 * therefore keeps every comparison, key and answer on the plain word and only
 * renders the vocalised one.
 *
 *   IAKidsNikud.of('שולחן')              // 'שׁוּלְחָן', or '' when unknown
 *   IAKidsNikud.text('הכלב רץ')          // whole phrase, word by word
 *   IAKidsNikud.render(el, 'שולחן')      // sets el's text, marking letter i in <b> with opts.mark
 *   IAKidsNikud.local({ 'שמן': 'שָׁמֵן' })  // this game means the other homograph
 *
 * Without nikud.js loaded every call falls back to the plain word, so a game
 * that forgets the <script> tag still works.
 */
const IAKidsNikud = {
  _local: {},
  ...
  // A phrase: each Hebrew run is looked up on its own, everything else is kept.
  text(s) {
    return String(s).replace(/[\u05D0-\u05EA]+/g, w => this.of(w) || w);
  },
  // Split a vocalised word into clusters of "letter + the marks that follow it".
  clusters(voc) { return String(voc).match(/[^\u0591-\u05C7][\u0591-\u05C7]*/g) || []; },
  /**
   * Write `word` into `el` with nikud. opts.mark is the index of a letter to
   * emphasise (`<b>`), counted on the plain word — the marks never shift it.
   */
  render(el, word, opts = {}) {
    if (!el) return;
    const plain = this.strip(word), voc = this.of(plain) || plain;
    el.textContent = '';
    const cl = this.clusters(voc);
    if (opts.mark == null || opts.mark < 0 || opts.mark >= cl.length) { el.textContent = voc; return; }
    cl.forEach((c, i) => {
      const node = document.createElement(i === opts.mark ? 'b' : 'span');
      node.textContent = c;
      el.appendChild(node);
    });
  },
