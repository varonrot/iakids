...
async def homework_analyze(
    ...
    try:
        ...
        detected_topic = hebrew_page_label(detected_topic, analysis.get("page_title"), extracted_text)
...
HOMEWORK_PLAN_WAIT_SECONDS = float(os.getenv("HOMEWORK_PLAN_WAIT_SECONDS", "12"))
...
def hebrew_page_label(value, page_title, extracted_text) -> str:
...
async def plan_homework_page(analysis: dict, grade, on_batch=None) -> dict:
    ...
    batches = [texts[i:i + HOMEWORK_PLAN_BATCH] for i in range(0, len(texts), HOMEWORK_PLAN_BATCH)] or [None]
...
async def homework_plan_block_for(user_id: str, upload_id: str | None, current_question: str) -> str:
    """The plan block for the question on screen, or "" (no upload id, plan failed, no match).
    Waits up to HOMEWORK_PLAN_WAIT_SECONDS while the planner is still running."""
    if not upload_id:
        return ""
    ...
    return homework_plan_block(plan.get("page_summary", ""), q)
...
async def homework_coach(
    ...
    if mode_instruction:
        system_prompt += "\n\n" + mode_instruction
    plan_block = await homework_plan_block_for(user.id, req.upload_id, req.current_question)
    if plan_block:
        system_prompt += "\n\n" + plan_block
