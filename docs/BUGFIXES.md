...
- **Unbounded model spend (high)**: the per-minute limit was keyed on the raw header (`Bearer Bearer <t>` or any random header = a new bucket) and paid routes had no per-user cap. **Fix**: the limiter keys on the verified user id (IP otherwise), strict `Bearer <jwt>` parsing; a daily per-user budget (`DAILY_LIMIT_TTS/STT/MODEL/VISION`, defaults 1500/600/1000/60) on every model/TTS/STT/vision route incl. English and Test Prep; STT duration and size caps; clean-chat and coach-v2 are tagged in `ai_calls`, clip the browser history and accept only our Storage or inline images.
- **Medium/low**: regenerate-transition (changed a shared lesson for every child) is admin-only; admins must have signed in with Google (`ADMIN_REQUIRE_GOOGLE`, optional `ADMIN_USER_IDS`); the lesson quota fails closed and the "generating" claim is atomic (also fixes a lesson left on "generating" after a quota 403); children's text out of prod logs; generic TTS/STT errors; session ids checked for ownership; closing and homework fields are returned as plain text; English topic from a fixed list; age 1–18; kid-limit race; malformed/HS256/anonymous tokens refused before Supabase Auth; homework file size + type checked before vision.
- **Verified**: `tools/security_gate_tutor.py` (21 negative tests), `prompt_gate.py --all` and `install_hooks.sh` pass; deployed with `tools/deploy_tutor.sh` (health OK); on prod with a throwaway user: own data 200, a `..` path 403, age 0 → 422, non-admin on admin 403, forged/doubled bearer 401, `/docs` 404; the user was removed.
- **Gate**: `security_review_checks` in `prompt_gate.py` (tutor rules + the image/history rule); the pre-commit hook also triggers on tutor modules. Payments, pages and SQL fixes follow in their own commits.
- **Build**: 0.7.154.
