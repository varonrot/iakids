...
LESSON_CLOSING_PROMPT_PATH = Path(
    "prompts/iakids_lesson_closing_prompt.txt"
)
HOMEWORK_COACH_PROMPT_PATH = Path(
    "prompts/homework/iakids_homework_coach_prompt.txt"
)
HOMEWORK_PEDAGOGY_PROMPT_PATH = Path(
    "prompts/homework/iakids_homework_pedagogy_prompt.txt"
)
...
# Every model is read from the environment; the value after the name is the default used when
# the variable is missing from .env / the service unit (2026-09-23). On OpenRouter the OpenAI names
# get the "openai/" prefix automatically (llm_model below).

# מודל זול לפעולות שוטפות: צ'אט, המשך שיעור, הערכה
DEFAULT_OPENAI_MODEL = os.getenv("CHAT_MODEL", "gpt-4o-mini")

# המודל החזק: יצירת תוכן שיעור אוניברסלי שנשמר במטמון
UNIVERSAL_LESSON_MODEL = os.getenv("LESSON_MODEL", "gpt-5.6-sol")

# המורה של שיעורי הבית (homework-coach, homework-coach-v2) ובניית תוכנית ההוראה לכל שאלה
HOMEWORK_COACH_MODEL_NAME = os.getenv("HOMEWORK_COACH_MODEL", "gpt-5.6-sol")
HOMEWORK_PLANNER_MODEL_NAME = os.getenv("HOMEWORK_PLANNER_MODEL", "gpt-5.6-sol")
CLEAN_CHAT_MODEL_NAME = os.getenv("CLEAN_CHAT_MODEL", "gpt-5.6-sol")

# Gemini direct (used when TTS_PROVIDER=direct, and for images / video)
...
LESSON_CLOSING_PROMPT_TEMPLATE = (
    LESSON_CLOSING_PROMPT_PATH
    .read_text(
        encoding="utf-8"
    )
)
...
# Request-path caches and local token verification (request_cache.py, 2026-09-25): the login
# check and the child's row were two database round trips on every request.
import request_cache as _rc
_jwt_verifier = _rc.LocalJWTVerifier(SUPABASE_URL)
...
HOMEWORK_COACH_MODEL = llm_model(HOMEWORK_COACH_MODEL_NAME)
HOMEWORK_PLANNER_MODEL = llm_model(HOMEWORK_PLANNER_MODEL_NAME)
CLEAN_CHAT_MODEL = llm_model(CLEAN_CHAT_MODEL_NAME)
print(f"[config] AI_PROVIDER={AI_PROVIDER} TTS_PROVIDER={TTS_PROVIDER} chat={DEFAULT_OPENAI_MODEL} lesson={UNIVERSAL_LESSON_MODEL} "
      f"homework_coach={HOMEWORK_COACH_MODEL} homework_planner={HOMEWORK_PLANNER_MODEL} clean_chat={CLEAN_CHAT_MODEL}")
...
def tts_cache_key(text: str, gender: str | None = None) -> str:
    model = OPENROUTER_TTS_MODEL if TTS_PROVIDER == "openrouter" else GEMINI_TTS_MODEL
...
ops.gauges["request_cache"] = lambda: _rc.stats(_jwt_verifier)       # hit/miss per cache, local vs network logins
...
def enqueue_media_job(
    ...
    # The same dedupe key within 60 s gets the same live row back from the database anyway;
    # a lesson open re-sent three of these on every poll (request_cache.py, 2026-09-25).
    if dedupe_key:
        hit = _rc.media_enqueues.get(dedupe_key)
        if hit is not None:
            return hit
    ...
    job_id = int(res.data)
    if dedupe_key:
        _rc.media_enqueues.put(dedupe_key, job_id)
    return job_id
...
class HomeworkPlanStep(BaseModel):
    goal: str
    teacher_explains: str
    child_does: str


class HomeworkQuestionPlan(BaseModel):
    question_text: str
    what_is_asked: str
    key_info: str
    steps: list[HomeworkPlanStep]
    common_mistakes: list[str]
    answer_criteria: str
    correct_answer: str


class HomeworkPagePlan(BaseModel):
    page_summary: str
...
class LessonClosingResponse(BaseModel):
    """What the teacher says when the whole lesson ends.

    2026-09-17: until today a lesson ended with a 20 s video that is identical for
    every lesson and every child, and a card with three numbers. Nobody ever told the
    child what they had learned. `spoken` is read aloud, the three short lines are
    shown next to it, and parent_note is what the parent panel shows instead of a
    percentage.
    """
    spoken: str
    learned: str
...
def authenticate_user(authorization: str):
    ...
    local_user = _jwt_verifier.verify(token)
...
def get_child_by_id(
    ...
    # 60 s per (parent, child): every route checks the child belongs to the caller, and the row
    # changes only through /api/kid/update, which drops it (request_cache.py).
    cached = _rc.child_rows.get((user_id, kid_id))
    ...
    _rc.child_rows.put((user_id, kid_id), rows[0])
    return rows[0]
...
def build_lesson_closing_prompt(
        ...
        unit_lesson: dict,
        parts: list,
        answers_by_part: dict,
        scores_by_part: dict
) -> tuple[str, dict]:
    """The closing prompt plus the RUNTIME_DATA it was built from."""
    lesson_parts = []
    ...
    for part in parts:
        ...
        lesson_parts.append({
            ...
            "child_answers":
                answers_by_part.get(part_number) or [],

            "understanding_score":
                scores_by_part.get(part_number)
        })

    runtime_data = {
        "child": {
            "child_name":
                lq.display_first_name(
        ...
        "lesson": {
            ...
            "lesson_goal":
                (
                    ...
                    or parent_lesson.get("lesson_goal")
                )
        },
    ...
    return final_prompt, runtime_data


def find_cached_lesson_closing(
        kid_id: str,
        unit_lesson_id: int
) -> dict | None:
    """The closing is generated once per child per lesson and reused after that.

    It is stored as a normal history row whose `evaluation` carries the marker, so no
    new column and no migration are needed, and the parent panel - which already reads
    this table - sees it for free.
    """
    try:
        res = (
            sb.table("kid_lesson_history")
            .select("id, content, evaluation")
            .eq("kid_id", kid_id)
        ...
        print("LESSON CLOSING CACHE READ FAILED:", repr(cache_error))
        return None

    for row in (res.data or []):
        evaluation = row.get("evaluation") or {}
        if isinstance(evaluation, dict) and evaluation.get("kind") == LESSON_CLOSING_MARKER:
            return {
                "spoken": row.get("content") or "",
                "learned": evaluation.get("learned") or "",
                "did_well": evaluation.get("did_well") or "",
                "to_strengthen": evaluation.get("to_strengthen") or "",
                "parent_note": evaluation.get("parent_note") or "",
                "cached": True
            }
    return None


def collect_lesson_closing_inputs(
        kid_id: str,
        unit_lesson_id: int,
        lesson_parts_count: int
) -> tuple[dict, dict]:
    """(answers_by_part, scores_by_part) - what the child actually said and scored."""
    answers_by_part: dict = {}
    scores_by_part: dict = {}
    ...
    try:
        history = (
            sb.table("kid_lesson_history")
            .select("role, content, part_number, id")
            .eq("kid_id", kid_id)
            .eq("unit_lesson_id", unit_lesson_id)
            .eq("role", "user")
            .order("id")
        ...
        print("LESSON CLOSING HISTORY FAILED:", repr(history_error))
        history = []

    for row in history:
        part_number = int(row.get("part_number") or 1)
        text = str(row.get("content") or "").strip()
        if text:
            answers_by_part.setdefault(part_number, []).append(text)

    try:
        sessions = (
            sb.table("learning_coach_sessions")
            .select("coach_index, final_understanding_score, created_at")
            .eq("kid_id", kid_id)
            .eq("unit_lesson_id", unit_lesson_id)
        ...
        print("LESSON CLOSING SESSIONS FAILED:", repr(sessions_error))
        sessions = []

    for row in sessions:
        coach_index = int(row.get("coach_index") or 1)
        if coach_index not in scores_by_part:
            scores_by_part[coach_index] = int(row.get("final_understanding_score") or 0)

    for part_number in range(1, max(1, int(lesson_parts_count or 1)) + 1):
        scores_by_part.setdefault(part_number, 0)

    return answers_by_part, scores_by_part
...
def _get_learning_lesson_uncached(
        lesson_id: int
):
...
def get_learning_lesson(
    ...
    """Cached 5 min (request_cache.py): curriculum rows do not change during a session."""
    hit = _rc.curriculum_rows.get(("learning_lesson", lesson_id))
    if hit is not None:
        return hit
    row = _get_learning_lesson_uncached(lesson_id)
    _rc.curriculum_rows.put(("learning_lesson", lesson_id), row)
    return row
    ...
    def create_session():
        ...
        return (
            ...
            .insert({
                ...
                "tts_model":
                    OPENROUTER_TTS_MODEL if TTS_PROVIDER == "openrouter" else GEMINI_TTS_MODEL
            })
            .execute()
        )
...
def _is_paid_active_subscription_uncached(
        user_id: str
) -> bool:
    """
...
def is_paid_active_subscription(
    ...
    """Cached 60 s per parent (request_cache.py): every lesson open asked the database."""
    hit = _rc.subscriptions.get(user_id)
    if hit is not None:
        return hit
    paid = _is_paid_active_subscription_uncached(user_id)
    _rc.subscriptions.put(user_id, paid)
    return paid
...
LESSON_TRANSITION_VIDEO_MODEL = os.getenv("LESSON_TRANSITION_VIDEO_MODEL", "gemini-omni-1.1-flash")
...
LESSON_IMAGE_MODEL = os.getenv("LESSON_IMAGE_MODEL", "gemini-3.1-flash-lite-image")
    ...
    try:
        ...
        try:
            ...
            if personal_intro_eligible:
                ...
                ready_intro_videos = (await run_in_threadpool(lambda: get_ready_kid_lesson_intro_videos(
                    child["id"]
                )))
        ...
        if (
            ...
            return {
                ...
                "transition":
                    (await run_in_threadpool(lambda: add_transition_video_signed_url(
                        cached_json.get(
                            "transition"
                        )
                    ))),
...
# UNIT LESSON CLOSING
#
# What the teacher says when the whole lesson is over. Generated once per child per
# lesson and then reused: re-entering a finished lesson must not cost another call.
# =====================================================

@app.post(
    "/api/tutor/unit-lesson/closing"
)
async def get_or_generate_unit_lesson_closing(
        body: UnitLessonRequest,
        authorization: str = Header(None)
):
    try:
        ...
        ai_context("lesson_closing", user, body)
        ...
        cached = (await run_in_threadpool(lambda: find_cached_lesson_closing(
            body.kid_id,
            body.unit_lesson_id
        )))
        ...
        structured_lesson = (
            (unit_lesson.get("generated_lesson_json") or {}).get("structured_lesson")
            or {}
        )

        parts = structured_lesson.get("parts") or []

        if not parts:
            raise HTTPException(
                status_code=409,
                detail="lesson has no parts yet"
            )

        answers_by_part, scores_by_part = (await run_in_threadpool(
            lambda: collect_lesson_closing_inputs(
                body.kid_id,
                body.unit_lesson_id,
                len(parts)
            )
        ...
        closing_prompt, runtime_data = build_lesson_closing_prompt(
            child=child,
            parent_lesson=parent_lesson,
            unit_lesson=unit_lesson,
            parts=parts,
            answers_by_part=answers_by_part,
            scores_by_part=scores_by_part
        )

        with media_trace.stage("lesson_closing", lesson=body.unit_lesson_id):
            completion = await aclient.beta.chat.completions.parse(
                model=llm_model(DEFAULT_OPENAI_MODEL),
                messages=[
                    {"role": "system", "content": closing_prompt},
                    {"role": "user", "content": "סכם את השיעור לילד עכשיו."}
                ],
                response_format=LessonClosingResponse
            )

        closing = completion.choices[0].message.parsed

        if not closing or not (closing.spoken or "").strip():
            raise HTTPException(
                status_code=502,
                detail="closing generation returned nothing"
            )

        payload = {
            "spoken":
                closing.spoken.strip(),

            "learned":
                (closing.learned or "").strip(),

            "did_well":
                (closing.did_well or "").strip(),

            "to_strengthen":
                (closing.to_strengthen or "").strip(),
        ...
        def store():
            sb.table("kid_lesson_history").insert({
                "kid_id": body.kid_id,
                "lesson_id": unit_lesson["learning_lesson_id"],
                "unit_lesson_id": body.unit_lesson_id,
                "session_id": None,
                "objective_index": None,
                "part_number": len(parts),
                "role": "assistant",
                "content": payload["spoken"],
                "evaluation": {"kind": LESSON_CLOSING_MARKER, **payload},
                "sequence_json": None
            }).execute()
        ...
        try:
            ...
            # A closing the child already heard is worth more than a stored copy.
            print("LESSON CLOSING STORE FAILED:", repr(store_error))

        print("LESSON CLOSING GENERATED:", {
            "unit_lesson_id": body.unit_lesson_id,
            "parts": len(parts),
            "spoken_chars": len(payload["spoken"])
        })
        ...
        print("UNIT LESSON CLOSING ERROR:", repr(e))
        traceback.print_exc()
        raise HTTPException(
            status_code=500,
            detail="closing failed"
        )
...
async def run_learning_coach(
    ...
    if coach_finished:
        ...
        lesson_xp_reward = int(
            (lesson or {}).get("xp_reward")
            or 0
        )

        lesson_stars_reward = int(
            (lesson or {}).get("stars_reward")
            or 0
        )
        ...
        progress_update = (await run_in_threadpool(lambda: (
            ...
            .update({
                ...
                # 2026-09-17: this stayed "in_progress" forever and completed_at,
                # xp_earned and stars_earned were never written, so the child's and
                # the parent's dashboards showed zero for every finished unit lesson.
                "status":
                    "in_progress"
                    if has_next_part
                    else "completed",
                ...
                **(
                    ...
                    if has_next_part
                        ...
                        "stars_earned": lesson_stars_reward
                    }
                )
            })
            .eq(
...
async def homework_analyze(
    ...
    try:
        ...
        # stage 2 (teaching plan) runs after the row is stored; "pending" tells the coach to wait for it
        plan_wanted = (HOMEWORK_PLANNER_ENABLED and not needs_high_resolution
                       and bool(analysis.get("exercises") or extracted_text.strip()))
        if plan_wanted:
            analysis["plan_status"] = "pending"
        # =============================================
        ...
        if plan_wanted:
            start_homework_planner(upload_row_id, analysis, child.get("grade"))
        # =============================================
        # SESSION USAGE
        # =============================================
...
# moved to a file on 2026-09-23 and rewritten without slash forms (הילד/ה, אומר/ת): they were read aloud
HOMEWORK_GLOBAL_PEDAGOGY_PROMPT = HOMEWORK_PEDAGOGY_PROMPT_PATH.read_text(encoding="utf-8").strip()
...
# what the child "says" when a help button opens the conversation (no typed message yet)
HOMEWORK_COACH_OPENERS = {
    "solve_together": "תלמדי אותי לפתור את השאלה הזאת כמו מורה פרטית. קודם תסבירי לי בקצרה מה השאלה מבקשת ואיך ניגשים אליה. אחר כך תני לי רק את הצעד הראשון, הסבירי מה עושים בו ולמה, ושאלי אותי שאלה קצרה אחת. בכל המשך תשמרי את מה שכבר עשיתי ותעזרי לי להתקדם צעד אחד בכל פעם עד שאוכל לענות בעצמי.",
    "understand_question": "אני רוצה להבין מה מבקשים בשאלה הזאת. אל תפתרי אותה, רק תסבירי לי מה צריך לעשות.",
    "explain_topic": "אני רוצה הסבר על החומר שצריך בשביל השאלה הזאת, ואז לחזור לשאלה.",
    "hint": "אני רוצה רמז קטן לשאלה הזאת, בלי התשובה.",
    "check_answer": "אני רוצה לבדוק תשובה שכתבתי לשאלה הזאת.",
}


# =====================================================
# HOMEWORK TEACHING PLAN (2026-09-23)
# Stage 1 reads the page (vision), stage 2 plans every question with the strong model, in the
# background. The plan stays in homework_uploads.analysis_json and is added to the teacher's
# prompt server side: the browser never receives it (it carries the correct answer for checking).
# =====================================================

HOMEWORK_PLANNER_ENABLED = os.getenv("HOMEWORK_PLANNER", "1") == "1"
# Stage 1 (reading the page). Measured 2026-09-23 on two real pages, 2 interleaved runs each:
# gpt-4o-mini read 10 of 20 exercises both times and none of the pictures; gemini-3.1-flash-lite read
# 20/20 and every sign in 3-5 s; gpt-5.6-sol read them too but took 25-92 s.
HOMEWORK_VISION_MODEL = os.getenv("HOMEWORK_VISION_MODEL") or (
    "google/gemini-3.1-flash-lite" if AI_PROVIDER == "openrouter" else DEFAULT_OPENAI_MODEL)
HOMEWORK_PLAN_WAIT_SECONDS = float(os.getenv("HOMEWORK_PLAN_WAIT_SECONDS", "15"))
_homework_plan_tasks: set = set()


def _hw_match_key(text: str) -> str:
    return re.sub(r"[^\w+\-×÷=*/]", "", str(text or "").lower().replace("_", ""))


def parse_homework_vision_json(raw: str) -> dict | None:
    """The page reader's JSON, tolerant of markdown fences and text around the object; None if
    it does not parse."""
...
def homework_reply_answer_items(reply: str, correct_answer: str, question: str) -> list:
    """Items of the correct answer that show up in the teacher's reply but not in the question.
    2026-09-23: on a Hasmonean timeline the teacher's "example" was "גזר ויפו" — one of the answers.
    For "X – Y" pairs (matching, who-did-what) only Y is the answer; X is given on the page."""
    ans = str(correct_answer or "").strip()
    if not ans:
        return []
    norm = lambda t: " " + " ".join(re.sub(r"[^\w\s]", " ", strip_nikud(str(t or "")).lower()).split()) + " "
    body, q = norm(reply), norm(question)
    if re.fullmatch(r"-?\d+(?:[.,]\d+)?", ans.strip(".")):
        n = ans.strip(".")
        hit = re.search(r"(?<![\d.,])" + re.escape(n) + r"(?![\d]|[.,]\d)", str(reply)) is not None
        return [n] if hit and not re.search(r"(?<![\d.,])" + re.escape(n) + r"(?![\d])", str(question)) else []
    items = []
    for part in re.split(r"[;\n]|,(?!\d)", ans):
        part = re.split(r"\s[–—-]\s", part)[-1]
        item = norm(part).strip()
...
def hebrew_page_label(value, page_title, extracted_text, subject: str = "") -> str:
    ...
    An English lesson keeps its topic ("Future tense" was blanked on 2026-09-23)."""
    value = str(value or "").strip()
    if subject == "אנגלית":
        return value
    if not re.search(r"[\u05D0-\u05EA]", str(extracted_text or "")) or re.search(r"[\u05D0-\u05EA]", value):
        return value
    title = str(page_title or "").strip()
    return title if re.search(r"[\u05D0-\u05EA]", title) else ""


HOMEWORK_SUBJECT_HE = {
    "math": "חשבון", "maths": "חשבון", "mathematics": "חשבון", "arithmetic": "חשבון",
    "english": "אנגלית", "science": "מדעים", "hebrew": "עברית", "language": "עברית",
    "reading comprehension": "הבנת הנקרא", "geography": "גאוגרפיה", "history": "היסטוריה",
}


def hebrew_subject_label(value, extracted_text) -> str:
    """The subject in Hebrew on a Hebrew page ("Math" -> "חשבון"); unknown English stays as is."""
    value = str(value or "").strip()
    if not re.search(r"[\u05D0-\u05EA]", str(extracted_text or "")) or re.search(r"[\u05D0-\u05EA]", value):
        return value
    return HOMEWORK_SUBJECT_HE.get(value.lower(), value)


def match_homework_question(questions: list, current_question: str) -> dict | None:
    """The plan entry for the question on screen. The screen may have split the page itself, so
    match on the text (exact, then containment, then word overlap), never on position alone."""
    cur = _hw_match_key(current_question)
    if not cur or not questions:
        return None
    keyed = [(q, _hw_match_key(q.get("question_text"))) for q in questions if isinstance(q, dict)]
    for q, k in keyed:
        if k and k == cur:
            return q
    for q, k in keyed:
        if len(k) >= 4 and len(cur) >= 4 and (k in cur or cur in k):
            return q
    words = lambda t: set(re.findall(r"[\w+\-×÷=]+", str(t or "").lower()))
    cw = words(current_question)
    best, best_score = None, 0.0
    for q, _ in keyed:
        qw = words(q.get("question_text"))
        if qw and cw:
...
def homework_plan_leaks(question_plan: dict) -> bool:
    """True when the correct answer shows up in what the teacher is told to SAY (steps, key_info,
    what_is_asked, common_mistakes) — the teacher would read the answer to the child."""
    ans = str((question_plan or {}).get("correct_answer") or "").strip().strip(".").strip()
    if not ans:
        return False
    said = [question_plan.get("what_is_asked"), question_plan.get("key_info")]
    said += list(question_plan.get("common_mistakes") or [])
    for st in question_plan.get("steps") or []:
        said += [st.get("goal"), st.get("teacher_explains"), st.get("child_does")]
    blob = " ".join(str(x or "") for x in said)
    if re.fullmatch(r"-?\d+(?:[.,]\d+)?", ans):
        return re.search(r"(?<![\d.,])" + re.escape(ans) + r"(?![\d]|[.,]\d)", blob) is not None
    norm = lambda t: " ".join(re.sub(r"[^\w\s]", " ", t.lower()).split())
    a = norm(ans)
    return len(a) >= 2 and f" {a} " in f" {norm(blob)} "


def homework_plan_block(page_summary: str, question_plan: dict) -> str:
    """The plan as the teacher sees it. Internal: it must never be quoted to the child."""
    q = question_plan or {}
    steps = "\n".join(
        f"{i}. {s.get('goal', '')} — להסביר: {s.get('teacher_explains', '')} — הילד: {s.get('child_does', '')}"
        for i, s in enumerate(q.get("steps") or [], 1)
    ) or "(no steps: teach from what_is_asked and answer_criteria)"
    mistakes = "; ".join(str(m) for m in (q.get("common_mistakes") or [])) or "-"
    return (
        "HOMEWORK TEACHING PLAN — internal, built from reading the whole page. Follow it; never show it, "
        "never quote it, never read it aloud.\n"
        f"Page: {page_summary or '-'}\n"
        f"What the question asks: {q.get('what_is_asked', '-')}\n"
        f"Where the information is: {q.get('key_info', '-')}\n"
        f"Steps, in order — continue from the first step the child has not completed yet. The steps are a "
        f"path, not a gate: if the child gives the correct final answer at any point, it is correct — "
        f"confirm it and finish the question; never send the child back to a step:\n{steps}\n"
        f"Typical mistakes to watch for: {mistakes}\n"
        f"A sufficient answer contains: {q.get('answer_criteria', '-')}\n"
        f"CORRECT ANSWER — only to check the child's answer. Never say it, never hint at its digits or "
        f"words, never confirm a wrong answer: {q.get('correct_answer') or '(open question: judge by the criteria)'}"
    )


HOMEWORK_PLAN_BATCH = int(os.getenv("HOMEWORK_PLAN_BATCH", "4"))
...
async def _plan_homework_batch(page: dict, grade, plan_only: list | None, system_prompt: str = "") -> dict:
    ...
    for attempt in (1, 2):
        ...
        leaks = [q["question_text"] for q in plan["questions"] if homework_plan_leaks(q)]
        if not leaks:
            break
        print("HOMEWORK PLAN LEAK (attempt %d):" % attempt, leaks[:5])
    if plan is None:
        raise RuntimeError("homework planner returned no plan")
    removed = 0
    for q in plan["questions"]:
        if homework_plan_leaks(q):
            q["steps"], q["key_info"], q["common_mistakes"] = [], "", []
            removed += 1
    plan["leaks_removed"] = removed
    return plan


async def plan_homework_page(analysis: dict, grade, on_batch=None) -> dict:
    """Stage 2: plan every question on the page. A 20-exercise page took 148 s in one call
    (2026-09-23), so the exercises are planned in parallel batches; on_batch(plan_so_far) lets the
    caller store each batch as it lands, so question 1 does not wait for question 20."""
    page = {k: analysis.get(k) for k in ("subject", "topic", "language", "page_title", "task_context",
                                          "sections", "instructions", "exercises", "extracted_text")}
    texts = [str(e.get("text")).strip() for e in (analysis.get("exercises") or [])
             if isinstance(e, dict) and str(e.get("text") or "").strip()]
    # question 1 alone in the first batch: the child starts there, and a one-question call lands first
    ...
    total = {"page_summary": "", "questions": [], "leaks_removed": 0}
    gate = asyncio.Semaphore(HOMEWORK_PLAN_PARALLEL)
    lock = asyncio.Lock()
    ...
    async def one(index, plan_only):
        ...
        async with lock:
            ...
            total["page_summary"] = total["page_summary"] or part.get("page_summary", "")
            total["questions"] = [q for k in sorted(done) for q in done[k]["questions"]]
            total["leaks_removed"] = sum(d.get("leaks_removed", 0) for d in done.values())
            total["batches_done"], total["batches"] = len(done), len(batches)
            if on_batch:
                await on_batch(dict(total))

    results = await asyncio.gather(*(one(i, b) for i, b in enumerate(batches)), return_exceptions=True)
    failed = [r for r in results if isinstance(r, Exception)]
    if failed and not total["questions"]:
        raise failed[0]
    if failed:
        print("HOMEWORK PLAN PARTIAL:", {"failed_batches": len(failed), "error": repr(failed[0])[:160]})
    return total


async def _run_homework_planner(upload_id: str, analysis: dict, grade):
    started = time.time()

    async def store(plan, status):
        merged = {**analysis, "teaching_plan": plan, "plan_status": status}
        try:
            await run_in_threadpool(lambda: sb.table("homework_uploads").update(
                {"analysis_json": merged}).eq("id", upload_id).execute())
        except Exception as e:
            print("HOMEWORK PLAN STORE FAILED:", {"upload_id": upload_id, "error": repr(e)[:200]})

    async def on_batch(plan_so_far):
        if plan_so_far.get("batches_done", 0) < plan_so_far.get("batches", 0):
            await store(plan_so_far, "pending")                  # questions planned so far are usable now

    try:
        plan = await plan_homework_page(analysis, grade, on_batch=on_batch)
        await store(plan, "ready")
        print("HOMEWORK PLAN READY:", {"upload_id": upload_id, "questions": len(plan["questions"]),
                                       "batches": plan.get("batches"), "leaks_removed": plan["leaks_removed"],
                                       "seconds": round(time.time() - started, 1)})
    except Exception as e:
        await store({}, "failed")
        print("HOMEWORK PLAN FAILED:", {"upload_id": upload_id, "error": repr(e)[:200]})


def start_homework_planner(upload_id: str, analysis: dict, grade) -> bool:
    """Fire-and-forget: the child sees the page at once; the plan is ready by the first help turn."""
    if not HOMEWORK_PLANNER_ENABLED or not upload_id:
        return False
    task = asyncio.create_task(_run_homework_planner(upload_id, dict(analysis), grade))
    _homework_plan_tasks.add(task)
    task.add_done_callback(_homework_plan_tasks.discard)
    return True
...
async def homework_plan_block_for(user_id: str, upload_id: str | None, current_question: str) -> tuple[str, dict, str | None]:
    ...
    while True:
        rows = await run_in_threadpool(lambda: sb.table("homework_uploads").select("analysis_json")
                                       .eq("id", upload_id).eq("user_id", user_id).limit(1).execute().data)
        analysis = (rows[0].get("analysis_json") if rows else None) or {}
        status = analysis.get("plan_status")
        plan = analysis.get("teaching_plan") or {}
        q = match_homework_question(plan.get("questions") or [], current_question)
...
async def homework_coach(
    ...
    system_prompt = hebrew_child_prompt_block(child) + "\n" + HOMEWORK_COACH_PROMPT_TEMPLATE
    # every help button of the workspace comes here (2026-09-23); the mode says what kind of help
    ...
    if req.message.strip():
        ...
        messages.append({"role":"user","content":HOMEWORK_COACH_OPENERS.get(mode_name or "", HOMEWORK_COACH_OPENERS["solve_together"])})
    ...
    leaked = homework_reply_answer_items(raw_text, plan_question.get("correct_answer"), req.current_question) if state == "continue" else []
    if leaked:
        print("HOMEWORK REPLY USED THE ANSWER - REWRITING:", {"items": leaked[:5]})
        response = (await aclient.chat.completions.create(
            model=HOMEWORK_COACH_MODEL,
            messages=messages + [{"role": "assistant", "content": raw_text},
                                 {"role": "system", "content": "The reply above used part of the answer (" + ", ".join(leaked[:5]) +
                                  "). Rewrite it for the child without any word, number or item from the answer — not as an example "
                                  "and not as an option to choose from. Start with [[CONTINUE]]."}]
        ))
        raw_text = str(response.choices[0].message.content or "").strip()
        state = "complete" if raw_text.startswith("[[COMPLETE]]") else "continue"
    ...
    return {"reply": text, "state": state, "model": HOMEWORK_COACH_MODEL, "production_mode": True,
            "help_mode": mode_name, "plan_used": bool(plan_block)}
...
def update_my_kid(body: KidUpdateRequest, authorization: str = Header(None)):
    ...
    _rc.child_rows.drop(lambda k: k == (user.id, body.kid_id))   # the next read sees the new name / gender
