...
# The new parser uses re. main.py did not previously need it in this area.
if '\nimport re\n' not in backend[:1200]:
    backend = backend.replace('import math\n', 'import math\nimport re\n', 1)

# Ask the model for strict JSON mode as well. Apply only inside homework vision calls.
...
if needle in backend:
    backend = backend.replace(needle, replacement)

# Strengthen the prompt for non-homework/random images too.
...
# Visible application build bump.
index = re.sub(r'IAKIDS • build 0\\.7\\.\\d+', 'IAKIDS • build 0.7.72', index, count=1)
index = re.sub(r'window\\.IAKIDS_BUILD_VERSION = "0\\.7\\.\\d+";', 'window.IAKIDS_BUILD_VERSION = "0.7.72";', index, count=1)

BACKEND.write_text(backend, encoding='utf-8')
PROMPT.write_text(prompt, encoding='utf-8')
INDEX.write_text(index, encoding='utf-8')

print(f'Patched homework vision JSON parsing ({count} parse block(s)); build 0.7.72')
